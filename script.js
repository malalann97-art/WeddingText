...
function readOptions() {
  return {
    ...
    ai: {
      ...
      model: $("aiModel")?.value || (window.DEEPSEEK_CONFIG?.model || "deepseek-chat"),
...
async function callDeepseek(options) {
  const { ai } = options;
  if (!ai?.apiKey) throw new Error('未填写 API Key');
  const { system, user } = buildPrompt(options);
  const sanitizeAscii = (s) => String(s || '')
    // 去掉零宽、中文、emoji 等非 ASCII
    .replace(/[^\x00-\x7F]/g, '')
    // 去掉控制符与换行，避免 header 注入
    .replace(/[\r\n\t]/g, '')
    .trim();
  const apiKeyAscii = sanitizeAscii(ai.apiKey);

  // 方案 A：Plain Object（部分环境兼容性更好）
  const headersObj = {
    'content-type': 'application/json',
    'authorization': 'Bearer ' + apiKeyAscii,
  };
  // 方案 B：Headers 对象
  let headersInst = undefined;
  try {
    headersInst = new Headers();
    headersInst.set('content-type', 'application/json');
    headersInst.set('authorization', 'Bearer ' + apiKeyAscii);
  } catch (e) {
    headersInst = undefined;
  }
  // 通用 POST(JSON) 回退：XMLHttpRequest，以绕过某些浏览器对 fetch headers 的编码限制
  async function postJsonWithXHR(url, headersKV, jsonBody) {
    const body = JSON.stringify(jsonBody);
    const responseText = await new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', url, true);
      try {
        (headersKV || []).forEach(([k, v]) => xhr.setRequestHeader(k, v));
      } catch (_) {}
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status >= 200 && xhr.status < 300) resolve(xhr.responseText);
          else reject(new Error('XHR ' + xhr.status + ': ' + (xhr.responseText || '')));
        }
      };
  ...
  if (!resp) {
    // 尝试 C：数组形式
    const altHeaders = [ ['content-type', 'application/json'], ['authorization', 'Bearer ' + apiKeyAscii] ];
    try {
      resp = await fetch('https://api.deepseek.com/chat/completions', {
        method: 'POST',
        headers: altHeaders,
      ...
      // 最终方案：使用 XHR 回退
      const data = await postJsonWithXHR('https://api.deepseek.com/chat/completions', altHeaders, {
        model: ai.model || 'deepseek-chat',
        messages: [
          { role: 'system', content: system },
          { role: 'user', content: user },
  ...
  if (!resp.ok) {
    ...
    throw new Error(`DeepSeek 调用失败: ${resp.status} ${txt}`);
  }
